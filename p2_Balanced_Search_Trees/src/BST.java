...
public class BST<K extends Comparable<K>, V> implements STADT<K,V> {
  ...
  private int size = 0;
    ...
    private Node<K, V> getNodeWith(Node<K, V> root, K key){
      ...
      if(root.key.compareTo(key) > 0) return getNodeWith(root.left, key);
      if(root.key.compareTo(key) < 0) return getNodeWith(root.right, key);
      return null;
    }
    ...
    public K getKeyOfLeftChildOf(K key) throws IllegalNullKeyException, KeyNotFoundException {
      ...
      if (keyNode == null) throw new KeyNotFoundException();
      if (keyNode.left == null) return null;
    ...
    public K getKeyOfRightChildOf(K key) throws IllegalNullKeyException, KeyNotFoundException {
      ...
      Node<K, V> keyNode = getNodeWith(root, key);
      if (keyNode == null) throw new KeyNotFoundException();
      if (keyNode.right == null) return null;
